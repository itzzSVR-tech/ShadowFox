...
Ensure all questions are unique, highly entertaining, accurate, and completely in the style of "${host.name}".`;
    ...
    app.post("/api/trivia/hint", async (req, res) => {
        try {
...
The raw correct answer (KEEP SECRET): "${correctAnswer}"`;
    ...
    app.post("/api/trivia/chat", async (req, res) => {
        try {
            const {
                messages = [],
                questionContext = "",
...
If applicable, make jokes or remarks about the question they are currently facing: "${questionContext}".`;
            ...
            // Standard content array format or simple prompt construction depending on what works best
            // We can construct a structured prompt showing history
